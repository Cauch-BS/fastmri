...
from refs.datasets import make_default_train_dataloader, make_default_val_dataloader
...
parser.add_argument(
    "-img",
    "--img_save_folder",
    default="CelebHQ_Model_generated_images",
    help="Path to save images generated by model",
)
...
ValDataLoaderConfig = {
    "dataloader_kwargs": {
        "batch_size": int(TrainBatchSize / 2),
        "shuffle": False,
        "num_workers": 4,
    }
}

eval_loader = make_default_val_dataloader(
    indir="celebhq/val_256/random_medium_256/",
    img_suffix=".png",
    out_size=256,
    **ValDataLoaderConfig,
)
...
summary(
    model,
    input_size=[(1, 3, 256, 256), (1, 1, 256, 256)],
...
def calc_curr_performance(model, valloader, epoch=0, full_dataset=True):
    ...
    for i, data in enumerate(tqdm(valloader)):
        ...
        img, mask = torch.Tensor(data["image"].to(device)), torch.Tensor(
            data["mask"].to(device)
        )
    ...
    for j in range(3):
        cv2.imwrite(
            img_save_folder_loc + "/" + "eval_input" + str(j) + ".png",
            cv2.cvtColor(
                masked_img[j].permute([1, 2, 0]).cpu().detach().numpy() * 255,
                cv2.COLOR_RGB2BGR,
            ),
        )
        cv2.imwrite(
            img_save_folder_loc + "/" + "eval_output" + str(j) + ".png",
            cv2.cvtColor(
                out[j].permute([1, 2, 0]).cpu().detach().numpy() * 255,
                cv2.COLOR_RGB2BGR,
            ),
        )
...
class HybridLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        losses = (
            l_lpips
            + (1 - self.alpha) * (nn.L1Loss()(x, y))
            + (self.alpha * (nn.MSELoss()(x, y)))
        )
...
while counter < 15:
    ...
    for i in range(3):
        cv2.imwrite(
            img_save_folder_loc + "/" + str(epoch) + "__" + str(i) + "_Input.png",
            cv2.cvtColor(
                inputs[index + i].permute([1, 2, 0]).cpu().detach().numpy() * 255,
                cv2.COLOR_RGB2BGR,
            ),
        )
        cv2.imwrite(
            img_save_folder_loc + "/" + str(epoch) + "__" + str(i) + "_Output.png",
            cv2.cvtColor(
                outputs[index + i].permute([1, 2, 0]).cpu().detach().numpy() * 255,
                cv2.COLOR_RGB2BGR,
            ),
        )
