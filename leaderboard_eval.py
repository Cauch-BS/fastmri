    ...
    with torch.no_grad():
        for i_subject in range(58):
            l_fname = os.path.join(
                args.leaderboard_data_path, "brain_test" + str(i_subject + 1) + ".h5"
            )
            y_fname = os.path.join(
                args.your_data_path, "brain_test" + str(i_subject + 1) + ".h5"
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="FastMRI challenge Leaderboard Image Evaluation",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    parser.add_argument("-g", "--GPU_NUM", type=int, default=0)
    parser.add_argument(
        "-lp", "--path_leaderboard_data", type=Path, default="/Data/leaderboard/"
    )
    ...
    parser.add_argument(
        ...
        "--path_your_data",
        type=Path,
        default="../result/test_FM/reconstructions_leaderboard/",
    )
