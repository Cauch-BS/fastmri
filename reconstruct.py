...
from utils.learning.test_part_fm import forward
import os
import time
...
def parse():
    ...
    parser.add_argument(
        "-g", "--GPU_NUM", type=int, default=0, help="GPU number to allocate"
    )
    ...
    parser.add_argument(
        "--out-chans", type=int, default=1, help="Size of output channels for network"
    )
    parser.add_argument(
        "--input_key", type=str, default="image_input", help="Name of input key"
    )
...
if __name__ == "__main__":
    ...
    args.forward_dir = (
        "../result" / args.net_name / "reconstructions_leaderboard" / "public"
    )
    ...
    args.forward_dir = (
        "../result" / args.net_name / "reconstructions_leaderboard" / "private"
    )
    print(f"Saved into {args.forward_dir}")
    forward(args)
